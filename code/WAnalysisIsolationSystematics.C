...
void plot(RooDataSet* dataSet,RooDataSet* mcWSet, RooDataSet* mcZSet, RooDataSet* mcJ1Set, RooDataSet* mcJ2Set, RooDataSet* mcJ3Set, 
		...
		double centralityLow, double centralityUpp, RooRealVar& muonMt, RooRealVar& centrality, TString sSel, TString sSel2 , std::ostream& outputData, bool addPercent = true, 
	...
	writeToSpreadsheet(outputData,iEta, iCentrality,sigEventsSub,errStatSub,sigEventsSubErr);
...
int WAnalysis(){
	...
	TString fileNameDataIn = "HardProbesFiles/HISingleMuonHardProbesData.03.26.2013";
	//data overlay
//	TString fileNameMCWIn = "MonteCarloFiles/Wmunu/HISingleMuonWmunuMCDataOverlay.2012.11.21";
	TString fileNameMCWIn = "MonteCarloFiles/Wmunu/HISingleMuonWmunuPYTHIADataOverlay.03.24.2013";
	//HIJING overlay
	TString fileNameMCZIn = "MonteCarloFiles/Zmumu/HISingleMuonZmumuPYTHIAHIJINGOverlay.03.28.2013";
...
//	TString fileNameBkgQCDIn = "background/fractionQCDEtaScaled.01.21.2013";
	TString fileNameBkgQCDIn = "background/fractionQCDEtaScaled.03.27.2013";
...
//	TString fileNameBkgQCDIn = "systematics/QCD/fractionQCDEtaScaled_1sigmaDownMPt.02.08.2013"; //mpt -1
    ...
    //REMEMBER TO UPDATE WITH 03.27.2013 FILES!!!
...
//	TString fileNameBkgZIn = "background/fractionZEtaCent.02.11.2013";
	TString fileNameBkgZIn = "background/fractionZEtaCent.03.28.2013";
	///files for pt,mpt cut systematics
...
//	TString fileNameCw = "systematics/eff/mptSigmaDown/correctionFactorsW_mpt1SigmaDown"; //-1mpt
	...
	TString spreadSheetName = "dataSpreadSheetIsolationConeSizeSystematic.csv";
	TString spreadSheetNameMuPlus = "dataSpreadSheetMuPlusIsolationConeSizeSystematic.csv";
	TString spreadSheetNameMuMinus = "dataSpreadSheetMuMinusIsolationConeSizeSystematic.csv";
	...
	spreadSheetMuMinus.open(spreadSheetNameMuMinus);
	  ...
	  //W selection cuts
	  TString sCutsSig = "muonPt>25.0&&missPt>25.0&&missPt<9000.0&&isolationMu<0.1&&muonMt>40.0&&ZDY==0";
	  ...
	  //TString sCutsSig = "muonPt>25.0&&missPt>(25.0-0.2)&&isolationMu<0.4&&muonMt>40.0&&ZDY==0";
	  RooFormulaVar cutsSig("cutsSig", "cutsSig", sCutsSig, RooArgList(muonPt,missPt,isolationMu,muonMt,ZDY));
...
//		double syst0 = yAx0[j]; double syst1 = yAx1[j]; double syst2 = yAx2[j]; 
        //hack applied when not using cW parametrization tech
		double syst0 = 0.0, syst1 = 0.0, syst2 = 0.0; 
...
//        double syst4 = yCwResid[k];
		...
		if(!doCentrality && !doEta && !doCharge){
			...
			plot(dataSubSet[i][j][k],mcWSubSet[i][j][k],mcZSubSet[i][j][k],mcJ1SubSet[i][j][k],mcJ2SubSet[i][j][k],mcJ3SubSet[i][j][k],
				...
				"#mu^{#pm},0-80" , "0 #leq |#eta| < 2.5" , spreadSheet, true,correctSpectra,doSubtractBkg,doPreSelKinematics);
			}
			...
			if(doPlotPhi){ 
					...
					99, 0, 0,nCentralityBins,0.0, 0.0, 0.35 , Cw,ncoll[k], ptBins[i], ptBins[i+1], etaBins[j], etaBins[j+1], centralityBins[k], centralityBins[k+1],isolationMu,
		...
		if(doCharge){
		   ...
		   RooDataSet* mcJ3SetMinus = (RooDataSet*) mcJ3SubSet[i][j][k]->reduce(Cut("chargeCategory==chargeCategory::muMinus"));
			...
			sSelPlus , sSelPlusEta,spreadSheetMuPlus, true,correctSpectra,doSubtractBkg,doPreSelKinematics);
			...
			sSelMinus , sSelMinusEta,spreadSheetMuMinus, true,correctSpectra,doSubtractBkg,doPreSelKinematics);
		    }
		    else {
			if(doPlotMt){
				std::cout << "plotting for mu+: " << i << ":" << j << ":" << k <<std::endl;
			...
			if(doPlotPhi){ 
					...
					102+i, j, k,nCentralityBins,0.0, 0.0, 0.35 , Cw,ncoll[k], ptBins[i], ptBins[i+1], etaBins[j], etaBins[j+1], centralityBins[k], centralityBins[k+1],isolationMu,
					...
					Cw,ncoll[k], ptBins[i], ptBins[i+1], etaBins[j], etaBins[j+1], centralityBins[k], centralityBins[k+1],isolationMu,
					"#mu^{-},0-80" , "0 #leq |#eta| < 2.5", 50.0,"#Sigma p^{ID}_{T}/p^{#mu}_{T}[GeV]", true, correctSpectra,doSubtractBkg,doPreSelKinematics);
			}
		   }
		...
		if (!doCharge&&doCentrality&&doEta) {
			...
			plot(dataSubSet[i][j][k],mcWSubSet[i][j][k],mcZSubSet[i][j][k],mcJ1SubSet[i][j][k],mcJ2SubSet[i][j][k],mcJ3SubSet[i][j][k], 
				...
				sSel , sSelEta, spreadSheet, true,correctSpectra,doSubtractBkg,doPreSelKinematics);
