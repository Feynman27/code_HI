  ...
  for(int iev = 0; iev < tree->GetEntries(); ++iev){
    ...
    for(int igen=0; igen<ngen; ++igen){
       ...
       if(
            ...
            // NOMINAL
            &&fabs(muonEtaNt[igen])>0.1&&fabs(muonEtaNt[igen])<2.5
            // CHANGED TO CALCULATE SCALE FACTOR TO COMBINE CHANNELS IN CENTRALITY
            //&&fabs(muonEtaNt[igen])>0.0&&fabs(muonEtaNt[igen])<2.5
...
//   std::cout << hname << " W-:" << hGenMinus->GetEntries() << " cross-section: " << csMinus << " scale factor: " << 1.0/(nEvents/2.)*csMinus << std::endl;
   std::cout << "MSTW W+ --> mu+: " << nWPos << std::endl;
   std::cout << "MSTW W- --> mu-: " << nWNeg << std::endl;
...
//   float nEvents = tree->GetEntries();
   ...
   for(int iev = 0; iev < tree->GetEntries(); ++iev){
    ...
    for(int igen=0; igen<ngen; ++igen){
        ...
        if(
            ...
            &&fabs(etaGen[igen])>0.1&&fabs(etaGen[igen])<2.5
            &&nuGenPt[igen]>25.0
            &&muGenPt[igen]>25.0
            &&mtTruth>40.0
   ...
   //hAsymm = getAsymmetry(hGenPlusc,hGenMinusc);
   //hAsymm = &(*hAsymmDummy);
   /*for(int i =1; i<hAsymm->GetNbinsX(); ++i){
     float asymm = (hGenPlusc->GetBinContent(i)-hGenMinusc->GetBinContent(i))/(hGenPlusc->GetBinContent(i)+hGenMinusc->GetBinContent(i));
     hAsymm->SetBinContent(i,asymm);
...
//        "ResultFiles/WAnalysis_fitResultCentChrgEta_AbsEtaBSCorr.08.26.2013";
        ...
        //"ResultFiles/WAnalysis_fitResultCentChrgEta_NewEtaBinning.11.27.2013";
...
//  TString fileNameMCWIn = baseString+"MonteCarloFiles/Wmunu/HISingleMuonWmunuPYTHIADataOverlay.03.24.2013";
  ...
  //TString filenameMSTWpp =  baseString+"MonteCarloFiles/Wmunu/PowhegPythia8_CT10NLO_pp_06.07.2014.root";
...
*/
    ...
    float arrBinning[] = {0.1,0.35,0.6,0.8,1.05,1.37,1.52,1.74,2.1,2.50};
	...
	if (doCentrality) {
		...
		npartBins.push_back(260.70);//10-20
...
*/
  ...
  std::cout << "number of centrality classes in input file: " << nWCentrality << std::endl;
  ...
  for (int i=0; i<nWEta; i++){
    ...
    for (int j=0; j<nWCentrality; j++){
      ...
      for (int k =0; k<nWpts; k++){
        ...
        // removed tracking efficiency syst
...
//   }      
 ...
 TFile *outFileTheory = new TFile("eta_theory_plots.root","RECREATE");
 TFile *fHisto = new TFile("muonAsymmetry.root","RECREATE");

 TList _grEtaDistroPlusCent;
 TList _grEtaDistroMinusCent;

 TH1D* arrhpD[nCentralityBins];
 TH1D* arrhmD[nCentralityBins];
 TH1D* arrhAsymmD[nCentralityBins];
 TH1D* arrhpMC[nCentralityBins];
 TH1D* arrhmMC[nCentralityBins];

 for (int i=0; i<nWCentrality; i++){
    // combine 3rd and 4th bin
    if(i<2){
        centralityLow[i] = centralityArr[i] - centralityL[i] ;
        centralityHigh[i] = centralityArr[i] + centralityH[i] ;

        std::cout << "CentralityLow = " << centralityLow[i] << std::endl;
        std::cout << "CentralityHigh = " << centralityHigh[i] << std::endl;
    }
    else if(i==2){
        double binMedian = (centralityArr[i]+centralityArr[i+1])/2.;
        double binLimitLow = centralityL[i]+centralityL[i+1];
        double binLimitUpp = centralityH[i]+centralityH[i+1];
        centralityLow[i] = binMedian-binLimitLow;
        centralityHigh[i] = binMedian+binLimitUpp;
        std::cout << "CentralityLow = " <<  centralityLow[i]<< std::endl;
        std::cout << "CentralityHigh = " << centralityHigh[i] << std::endl;
    }
    else if(i==3) continue;
    else {
        centralityLow[i-1] = centralityArr[i] - centralityL[i] ;
        centralityHigh[i-1] = centralityArr[i] + centralityH[i] ;
        std::cout << "CentralityLow = " << centralityLow[i-1] << std::endl;
        std::cout << "CentralityHigh = " << centralityHigh[i-1] << std::endl;
    }
...
double* yWPlusTotCent0_5 = new double[nEtaBins];
double* yWPlusTotCent5_10 = new double[nEtaBins];
double* yWPlusTotCent10_20 = new double[nEtaBins];
double* yWPlusTotCent20_40 = new double[nEtaBins];
double* yWPlusTotCent40_80 = new double[nEtaBins];

double* yWMinusTotCent0_5 = new double[nEtaBins];
double* yWMinusTotCent5_10 = new double[nEtaBins];
double* yWMinusTotCent10_20 = new double[nEtaBins];
double* yWMinusTotCent20_40 = new double[nEtaBins];
double* yWMinusTotCent40_80 = new double[nEtaBins];

if(doCentrality) {
  std::cout << "Creating histos for " << nCentralityBins << " centrality classes." << std::endl;
  for(int i=0; i<nCentralityBins; ++i){
    ...
    // histos of |eta| per centrality class
 ...
 for (int i=0; i<nWEta; i++){
  ...
  for (int i = 0; i<nWEta; i++){
     ...
     yWPlusTotCent0_5[i] = 0.;
     yWPlusTotCent5_10[i] = 0.;
     yWPlusTotCent10_20[i] = 0.;
     yWPlusTotCent20_40[i] = 0.;
     yWPlusTotCent40_80[i] = 0.;

     yWMinusTotCent0_5[i] = 0.;
     yWMinusTotCent5_10[i] = 0.;
     yWMinusTotCent10_20[i] = 0.;
     yWMinusTotCent20_40[i] = 0.;
     ...
     for (int j =0; j<nWCentrality; j++){
       ...
       if (arrWSig[index]) {
        ...
        for (int k = 0; k<nPts; k++) {
          ...
          if (yW0[k]>0 && (abs(yW0L[k])>0||yW0H[k]>0)){
	       ...
	       if(k == 102){
             ...
             // remove tracking eff syst
             eyWPlusSumSyst6[i] += TMath::Power(0.0*yW0Syst6[k],2);
            ...
            if(doCentrality){
              //0-5%
              if(j==0) {
                  yWPlusTotCent0_5[i] += yW0[k];
                  // divide by eta bin width

                  arrhpD[0]->SetBinContent(i+1,yWPlusTotCent0_5[i]/binWidthTemp); 
                  arrhpD[0]->SetBinError(i+1,TMath::Sqrt(yWPlusTotCent0_5[i])/binWidthTemp);

                  std::cout << "W+ in 0-5%:" << yWPlusTotCent0_5[i] << std::endl;
                  std::cout << "W+ in 0-5 divided by bw:" << yWPlusTotCent0_5[i]/binWidthTemp << std::endl;

              }
              if(j==1){
                  yWPlusTotCent5_10[i] += yW0[k];
                  arrhpD[1]->SetBinContent(i+1,yWPlusTotCent5_10[i]/binWidthTemp); 
                  arrhpD[1]->SetBinError(i+1,TMath::Sqrt(yWPlusTotCent5_10[i])/binWidthTemp);

                  std::cout << "W+ in 5-10%:" << yWPlusTotCent5_10[i] << std::endl;
                  std::cout << "W+ in 5-10 divided by bw:" << yWPlusTotCent5_10[i]/binWidthTemp << std::endl;

              }
              // combine 10-15 and 15-20
              if(j>=2&&j<4) {
                  yWPlusTotCent10_20[i] += yW0[k];

                  // Set bin content after summation
                  if(j==3){
                      arrhpD[2]->SetBinContent(i+1,yWPlusTotCent10_20[i]/binWidthTemp); 
                      arrhpD[2]->SetBinError(i+1,TMath::Sqrt(yWPlusTotCent10_20[i])/binWidthTemp);

                      std::cout << "W+ in 10-20%:" << yWPlusTotCent10_20[i] << std::endl;
                      std::cout << "W+ in 10-20 divided by bw:" << yWPlusTotCent10_20[i]/binWidthTemp << std::endl;
                  }

              }
              // 20-40%
              if(j==4) { 
                  yWPlusTotCent20_40[i] += yW0[k];
                  arrhpD[3]->SetBinContent(i+1,yWPlusTotCent20_40[i]/binWidthTemp); 
                  arrhpD[3]->SetBinError(i+1,TMath::Sqrt(yWPlusTotCent20_40[i])/binWidthTemp);

                  std::cout << "W+ in 20-40%:" << yWPlusTotCent20_40[i] << std::endl;
                  std::cout << "W+ in 20-40 divided by bw:" << yWPlusTotCent20_40[i]/binWidthTemp << std::endl;
              }
              // 40-80%
              if(j==5) { 
                  yWPlusTotCent40_80[i] += yW0[k];

                  arrhpD[4]->SetBinContent(i+1,yWPlusTotCent40_80[i]/binWidthTemp); 
                  arrhpD[4]->SetBinError(i+1,TMath::Sqrt(yWPlusTotCent40_80[i])/binWidthTemp);

                  std::cout << "W+ in 40-80%:" << yWPlusTotCent40_80[i] << std::endl;
                  std::cout << "W+ in 40-80 divided by bw:" << yWPlusTotCent40_80[i]/binWidthTemp << std::endl;
              }
        ...
        // remove tracking eff syst
		 eyWMinusSumSyst6[i] += TMath::Power(0.0*yW0Syst6[k],2);
        ...
        if(doCentrality) {
              ...
              //0-5%
              if(j==0) {
                  yWMinusTotCent0_5[i] += yW0[k];
                  // divide by eta bin width

                  arrhmD[0]->SetBinContent(i+1,yWMinusTotCent0_5[i]/binWidthTemp); 
                  arrhmD[0]->SetBinError(i+1,TMath::Sqrt(yWMinusTotCent0_5[i])/binWidthTemp);

                  std::cout << "W- in 0-5%:" << yWMinusTotCent0_5[i] << std::endl;
                  std::cout << "W- in 0-5 divided by bw:" << yWMinusTotCent0_5[i]/binWidthTemp << std::endl;

                  //asymmetry
                  double nSigPlusTemp = arrhmD[0]->GetBinContent(i+1)*binWidthTemp;
              ...
              if(j==1){
                  yWMinusTotCent5_10[i] += yW0[k];
                  arrhmD[1]->SetBinContent(i+1,yWMinusTotCent5_10[i]/binWidthTemp); 
                  arrhmD[1]->SetBinError(i+1,TMath::Sqrt(yWMinusTotCent5_10[i])/binWidthTemp);

                  std::cout << "W- in 5-10%:" << yWMinusTotCent5_10[i] << std::endl;
                  std::cout << "W- in 5-10 divided by bw:" << yWMinusTotCent5_10[i]/binWidthTemp << std::endl;


                  double nSigPlusTemp = arrhmD[1]->GetBinContent(i+1)*binWidthTemp;
              ...
              // combine 10-15 and 15-20
              if(j>=2&&j<4) {
                  yWMinusTotCent10_20[i] += yW0[k];

                  // Set bin content after summation
                  if(j==3){
                      arrhmD[2]->SetBinContent(i+1,yWMinusTotCent10_20[i]/binWidthTemp); 
                      arrhmD[2]->SetBinError(i+1,TMath::Sqrt(yWMinusTotCent10_20[i])/binWidthTemp);

                      std::cout << "W- in 10-20%:" << yWMinusTotCent10_20[i] << std::endl;
                      std::cout << "W- in 10-20 divided by bw:" << yWMinusTotCent10_20[i]/binWidthTemp << std::endl;

                      double nSigPlusTemp = arrhmD[2]->GetBinContent(i+1)*binWidthTemp;
              ...
              // 20-40%
              if(j==4) { 
                  yWMinusTotCent20_40[i] += yW0[k];

                  arrhmD[3]->SetBinContent(i+1,yWMinusTotCent20_40[i]/binWidthTemp); 
                  arrhmD[3]->SetBinError(i+1,TMath::Sqrt(yWMinusTotCent20_40[i])/binWidthTemp);

                  std::cout << "W+ in 20-40%:" << yWMinusTotCent20_40[i] << std::endl;
                  std::cout << "W+ in 20-40 divided by bw:" << yWMinusTotCent20_40[i]/binWidthTemp << std::endl;

                  double nSigPlusTemp = arrhmD[3]->GetBinContent(i+1)*binWidthTemp;
                  double nSigMinusTemp = arrhmD[3]->GetBinContent(i+1)*binWidthTemp;
                  double asymmTemp = ((nSigPlusTemp-nSigMinusTemp)/(nSigPlusTemp+nSigMinusTemp));
                  double err = TMath::Sqrt(nSigPlusTemp+nSigMinusTemp);
                  double errAsymm = TMath::Sqrt(TMath::Power(err/(nSigPlusTemp-nSigMinusTemp),2)+TMath::Power(err/(nSigPlusTemp+nSigMinusTemp),2))*asymmTemp;
                  arrhAsymmD[3]->SetBinContent(i+1,asymmTemp);
                  arrhAsymmD[3]->SetBinError(i+1,errAsymm);
              }
              // 40-80%
              if(j==5) { 
                  yWMinusTotCent40_80[i] += yW0[k];

                  arrhmD[4]->SetBinContent(i+1,yWMinusTotCent40_80[i]/binWidthTemp); 
                  arrhmD[4]->SetBinError(i+1,TMath::Sqrt(yWMinusTotCent40_80[i])/binWidthTemp);

                  std::cout << "W+ in 40-80%:" << yWMinusTotCent40_80[i] << std::endl;
                  std::cout << "W+ in 40-80 divided by bw:" << yWMinusTotCent40_80[i]/binWidthTemp << std::endl;

                  double nSigPlusTemp = arrhmD[4]->GetBinContent(i+1)*binWidthTemp;
                  double nSigMinusTemp = arrhmD[4]->GetBinContent(i+1)*binWidthTemp;
                  double asymmTemp = ((nSigPlusTemp-nSigMinusTemp)/(nSigPlusTemp+nSigMinusTemp));
                  double err = TMath::Sqrt(nSigPlusTemp+nSigMinusTemp);
                  double errAsymm = TMath::Sqrt(TMath::Power(err/(nSigPlusTemp-nSigMinusTemp),2)+TMath::Power(err/(nSigPlusTemp+nSigMinusTemp),2))*asymmTemp;
                  arrhAsymmD[4]->SetBinContent(i+1,asymmTemp);
                  arrhAsymmD[4]->SetBinError(i+1,errAsymm);
              }
...
//	writeYieldsToSpreadsheet(spreadSheetMuMinus,i, yWMinusTot[i] , eyWMinusH[i], eyWMinusSystH[i] );
  ...
  TGraphAsymmErrors* _grassCorrelatedSyst = new TGraphAsymmErrors(nEtaBins); 
...
//  }
  ...
  TString sEtaRange[] = {"0.1-0.35","0.35-0.6","0.6-0.8","0.8-1.05","1.05-1.37","1.37-1.52","1.52-1.74","1.74-2.1","2.1-2.5"};
...
*/
     ...
     */
     ...
     // keep correlated and uncorrelated systematics separate for combining e,mu channels
     double eSystH  = eAsymmUncorrelated;
     ...
     double eSystL  = eAsymmUncorrelated;
     //eSystL*=abs(assym);
	 ...
	 */
     ...
     _grassCorrelatedSyst->SetPoint(j,xEtaTemp,assym);
     ...
     _grassCorrelatedSyst->SetPointError(j,(etaBins[j+1]-etaBins[j])/2.0, (etaBins[j+1]-etaBins[j])/2.0,eSystL1,eSystH1);
  ...
  // NOMINAL ANALYSIS BINNING
  //float xBins[] = {0.1,0.35,0.6,0.8,1.05,1.37,1.52,1.74,2.1,2.50};
  // change binning to full range for electron combination in integrated yields
  float xBins[] = {0.1,0.35,0.6,0.8,1.05,1.37,1.52,1.74,2.1,2.50};
  float xBinsTheory[] = {0.1,0.35,0.6,0.8,1.05,1.37,1.52,1.74,2.1,2.50};
  int bins = sizeof(xBins)/sizeof(float)-1;
  int binsTheory = sizeof(xBinsTheory)/sizeof(float)-1;
  TH1F* hEtaPlusMcAll = new TH1F("hEtaPlusMcAll","hEtaPlusMcAll",binsTheory,xBinsTheory);
  hEtaPlusMcAll->Sumw2();
  TH1F* hEtaMinusMcAll = new TH1F("hEtaMinusMcAll","hEtaMinusMcAll",binsTheory,xBinsTheory);
  ...
  TH1F* hEtaPlusMSTWpp = new TH1F("hEtaPlusMSTWpp","hEtaPlusMSTWpp",binsTheory,xBinsTheory);
  hEtaPlusMSTWpp->Sumw2();
  TH1F* hEtaPlusMSTWnn = new TH1F("hEtaPlusMSTWnn","hEtaPlusMSTWnn",binsTheory,xBinsTheory);
  hEtaPlusMSTWnn->Sumw2();
  TH1F* hEtaPlusMSTWnppn = new TH1F("hEtaPlusMSTWnppn","hEtaPlusMSTWnppn",binsTheory,xBinsTheory);
  hEtaPlusMSTWnppn->Sumw2();
  TH1F* hEtaMinusMSTWpp = new TH1F("hEtaMinusMSTWpp","hEtaMinusMSTWpp",binsTheory,xBinsTheory);
  hEtaMinusMSTWpp->Sumw2();
  TH1F* hEtaMinusMSTWnn = new TH1F("hEtaMinusMSTWnn","hEtaMinusMSTWnn",binsTheory,xBinsTheory);
  hEtaMinusMSTWnn->Sumw2();
  TH1F* hEtaMinusMSTWnppn = new TH1F("hEtaMinusMSTWnppn","hEtaMinusMSTWnppn",binsTheory,xBinsTheory);
  hEtaMinusMSTWnppn->Sumw2();
  TH1F* hEtaPlusMSTW = new TH1F("hEtaPlusMSTW","hEtaPlusMSTW",binsTheory,xBinsTheory);
  hEtaPlusMSTW->Sumw2();
  TH1F* hEtaMinusMSTW = new TH1F("hEtaMinusMSTW","hEtaMinusMSTW",binsTheory,xBinsTheory);
  hEtaMinusMSTW->Sumw2();

  TH1F* hAsymmMcAll = new TH1F("hAsymmMcAll","hAsymmMcAll",binsTheory,xBinsTheory);
...
//  TH1F* hAsymmMSTW = new TH1F("hAsymmMSTW","hAsymmMSTW",bins,xBins);
  ...
  fillMcHistos(fileNameMCWIn,bins,xBinsTheory,hEtaPlusMcAll,hEtaMinusMcAll,WCrossSectionPlusPythiaLO,WCrossSectionMinusPythiaLO,"AllNucleons"); 
  ...
  hAsymm_pp = getAsymmetry(hEtaPlus_pp,hEtaMinus_pp);
  hAsymm_nn = getAsymmetry(hEtaPlus_nn,hEtaMinus_nn);
  ...
  */
  ...
  bool doWeightMc=true;
  if(!doWeightMc){
    std::cout << "WARNING: MC nucleon combinations not weighted." << std::endl;
    wt_nn=wt_pp=wt_nppn=1.0;
  }
  fillMSTWHistos(filenameMSTWnn,binsTheory,xBinsTheory,hEtaPlusMSTWnn,hEtaMinusMSTWnn,
                    wt_nn*WCrossSection_nnPlusMSTWnlo,wt_nn*WCrossSection_nnMinusMSTWnlo,"nnTemp");
  fillMSTWHistos(filenameMSTWpp,binsTheory,xBinsTheory,hEtaPlusMSTWpp,hEtaMinusMSTWpp,
                    wt_pp*WCrossSection_ppPlusMSTWnlo,wt_pp*WCrossSection_ppMinusMSTWnlo,"ppTemp");
  fillMSTWHistos(filenameMSTWnppn,binsTheory,xBinsTheory,hEtaPlusMSTWnppn,hEtaMinusMSTWnppn,
                    wt_nppn*WCrossSection_nppnPlusMSTWnlo,wt_nppn*WCrossSection_nppnMinusMSTWnlo,"nppnTemp");

  
  TH1F* hAsymmMSTWpp = getAsymmetry(hEtaPlusMSTWpp,hEtaMinusMSTWpp);
  TH1F* hAsymmMSTWnn = getAsymmetry(hEtaPlusMSTWnn,hEtaMinusMSTWnn);
  TH1F* hAsymmMSTWnppn = getAsymmetry(hEtaPlusMSTWnppn,hEtaMinusMSTWnppn);
  outFile->cd();
  hEtaPlusMSTWpp->Write("hEtaPlusMSTWpp"); hEtaMinusMSTWpp->Write("hEtaMinusMSTWpp"); hAsymmMSTWpp->Write("hAsymmMSTWpp");
  hEtaPlusMSTWnn->Write("hEtaPlusMSTWnn"); hEtaMinusMSTWnn->Write("hEtaMinusMSTWnn"); hAsymmMSTWnn->Write("hAsymmMSTWnn");
  hEtaPlusMSTWnppn->Write("hEtaPlusMSTWnppn"); hEtaMinusMSTWnppn->Write("hEtaMinusMSTWnppn"); hAsymmMSTWnppn->Write("hAsymmMSTWnppn");
...
*/
  ...
  hEtaMinusMSTW->Add(hEtaMinusMSTWnppn);
   ...
   std::cout << "MSTW W+ --> mu+ (AFTER CUTS): " << hEtaPlusMSTW->GetEntries()  << std::endl;
   std::cout << "MSTW W- --> mu- (AFTER CUTS): " << hEtaMinusMSTW->GetEntries() << std::endl;
...
//   for (int i=0;i<1;i++){
     ...
     hAsymm[0]->GetXaxis()->SetRangeUser(0.1,2.5);
   ...
   outFileTheory->cd(); grAsymmMcAll->Write("PYTHIALO_ETA_ASYMM");
...
//    grassDummy->Draw("pesame");
   ...
   leg->SetFillStyle(1001);
    ...
    grassSyst[0]->Write("grChargeAsymmStatUncorrelatedSyst");
    _grassCorrelatedSyst->Write("grChargeAsymmCorrelatedSyst");
...
*/
      ...
      grWpc->GetXaxis()->SetRangeUser(0.1,2.5);
...
//    c4->Print("chargeEtaDistr"/*fileNameDataIn+*/".pdf"); 
    ...
    hAssSyst->Write();
    outFile->cd();
    grWpUncorrelatedSystc->Write("sigEtaPlusUncorrelated");
    grWmUncorrelatedSystc->Write("sigEtaMinusUncorrelated");
    grWpCorrelatedSystc->Write("sigEtaPlusCorrelated");
    grWmCorrelatedSystc->Write("sigEtaMinusCorrelated");

    grWpc->Write("sigEtaPlus");
    grWmc->Write("sigEtaMinus");
    //hpMCtemp->Write();
    //hmMCtemp->Write();
...
*/
   ...
   std::cout << std::endl;
   std::cout << "Integrated data W+ (0.1-2.4) = " << hEtaPlusData->Integral() << std::endl;
   std::cout << "Integrated POWHEG (0.0-2.5) = " << hEtaPlusMSTW->Integral()<< std::endl;
   std::cout << "MC/Data integrated ratio: " << hEtaPlusMSTW->Integral()/hEtaPlusData->Integral() << std::endl;
   std::cout << std::endl;
   ...
   outFileTheory->cd(); grEtaPlusMcAll->Write("PYTHIALO_ETA_POS");
   ci = TColor::GetColor("#cc0000");
...
tex->SetTextSize(0.03846154);
   ...
   tex->Draw();
    ...
    hRatioMSTWPlusCorrelated->GetYaxis()->SetTitle("Data/NLO");
   ...
   std::cout << std::endl;
   std::cout << "Integrated data W- (0.1-2.4) = " << hEtaMinusData->Integral() << std::endl;
   std::cout << "Integrated POWHEG (0.0-2.5) = " << hEtaMinusMSTW->Integral()<< std::endl;
   std::cout << "MC/Data integrated ratio: " << hEtaMinusMSTW->Integral()/hEtaMinusData->Integral() << std::endl;
   std::cout << std::endl;
...
*/   
   ...
   */
   ...
   outFileTheory->cd(); grEtaMinusMcAll->Write("PYTHIALO_ETA_NEG");
   ci = TColor::GetColor("#0000ff");
...
tex->SetTextSize(0.03846154);
   ...
   hRatioMSTWMinusCorrelated->GetYaxis()->SetTitle("Data/NLO");
