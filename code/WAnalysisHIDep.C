...
class FitResult
{
  ...
  FitResult( const char* name, std::vector<double>& ptBins, std::vector<double>& etaBins, std::vector<double>& centralityBins ) : _name(name) {
	...
	// -- add objects to TList --//
        ...
        _sigSyst.Add( new TGraphAsymmErrors(nPtBins,x,y,xErrLow,xErrUpp) );
        _sigSyst1.Add( new TGraphAsymmErrors(nPtBins,x,y,xErrLow,xErrUpp) );
        _sigSyst2.Add( new TGraphAsymmErrors(nPtBins,x,y,xErrLow,xErrUpp) );
        _sigSyst3.Add( new TGraphAsymmErrors(nPtBins,x,y,xErrLow,xErrUpp) );
        _sigSyst4.Add( new TGraphAsymmErrors(nPtBins,x,y,xErrLow,xErrUpp) );
        _sigSyst5.Add( new TGraphAsymmErrors(nPtBins,x,y,xErrLow,xErrUpp) );
        _sigSyst6.Add( new TGraphAsymmErrors(nPtBins,x,y,xErrLow,xErrUpp) );
        _sigSyst7.Add( new TGraphAsymmErrors(nPtBins,x,y,xErrLow,xErrUpp) );
        _sigSyst8.Add( new TGraphAsymmErrors(nPtBins,x,y,xErrLow,xErrUpp) );
        _sigSyst9.Add( new TGraphAsymmErrors(nPtBins,x,y,xErrLow,xErrUpp) );
        _sigSyst10.Add( new TGraphAsymmErrors(nPtBins,x,y,xErrLow,xErrUpp) );

        _nmc.Add( new TGraphAsymmErrors(nPtBins,x,y,xErrLow,xErrUpp) );
        _nobs.Add( new TGraphAsymmErrors(nPtBins,x,y,xErrLow,xErrUpp) );
  ...
  ~FitResult() {
    ...
    for ( int i = 0; i < _sig.GetEntries(); i++ ) {
      ...
      if (_nobs.At(i))    delete _nobs.At(i);
      if (_nbkg.At(i))     delete _nbkg.At(i);
      if (_sig.At(i))     delete _sig.At(i);
      if (_sigSyst.At(i)) delete _sigSyst.At(i);
      if (_sigSyst1.At(i)) delete _sigSyst1.At(i);
      if (_sigSyst2.At(i)) delete _sigSyst2.At(i);
      if (_sigSyst3.At(i)) delete _sigSyst3.At(i);
      if (_sigSyst4.At(i)) delete _sigSyst4.At(i);
      if (_sigSyst5.At(i)) delete _sigSyst5.At(i);
      if (_sigSyst6.At(i)) delete _sigSyst6.At(i);
      if (_sigSyst7.At(i)) delete _sigSyst7.At(i);
      if (_sigSyst8.At(i)) delete _sigSyst8.At(i);
      if (_sigSyst9.At(i)) delete _sigSyst9.At(i);
      if (_sigSyst10.At(i)) delete _sigSyst10.At(i);
   ...
   /* 
    ...
    for ( int i = 0; i < _canvas.GetEntries(); i++ ) {
      if (_canvas.At(i)) delete _canvas.At(i);
    }*/
  ...
  void setPt( const int iPt, const int iEta, const int iCentrality, const double value ) {
    ...
    setX((TGraphAsymmErrors*)_nobs.At(index),iPt,value);
  ...
  void setNobs( const int iPt, const int iEta, const int iCentrality, const double value, const double errLow, const double errUpp ) {
    const int index = _nCentralityBins*iEta + iCentrality;
    setY( (TGraphAsymmErrors*)_nobs.At(index), iPt, value, errLow, errUpp );
  }
  ...
  void setSigSyst( const int iPt, const int iEta, const int iCentrality, const double value, const double errLow, const double errUpp ) {
    const int index = _nCentralityBins*iEta + iCentrality;
    setY( (TGraphAsymmErrors*)_sigSyst.At(index), iPt, value, errLow, errUpp );
  }
  void setSigSyst1( const int iPt, const int iEta, const int iCentrality, const double value, const double errLow, const double errUpp ) {
    const int index = _nCentralityBins*iEta + iCentrality;
    setY( (TGraphAsymmErrors*)_sigSyst1.At(index), iPt, value, errLow, errUpp );
  }
  void setSigSyst2( const int iPt, const int iEta, const int iCentrality, const double value, const double errLow, const double errUpp ) {
    const int index = _nCentralityBins*iEta + iCentrality;
    setY( (TGraphAsymmErrors*)_sigSyst2.At(index), iPt, value, errLow, errUpp );
  }
  void setSigSyst3( const int iPt, const int iEta, const int iCentrality, const double value, const double errLow, const double errUpp ) {
    const int index = _nCentralityBins*iEta + iCentrality;
    setY( (TGraphAsymmErrors*)_sigSyst3.At(index), iPt, value, errLow, errUpp );
  }
  void setSigSyst4( const int iPt, const int iEta, const int iCentrality, const double value, const double errLow, const double errUpp ) {
    const int index = _nCentralityBins*iEta + iCentrality;
    setY( (TGraphAsymmErrors*)_sigSyst4.At(index), iPt, value, errLow, errUpp );
  }
  void setSigSyst5( const int iPt, const int iEta, const int iCentrality, const double value, const double errLow, const double errUpp ) {
    const int index = _nCentralityBins*iEta + iCentrality;
    setY( (TGraphAsymmErrors*)_sigSyst5.At(index), iPt, value, errLow, errUpp );
  }
  void setSigSyst6( const int iPt, const int iEta, const int iCentrality, const double value, const double errLow, const double errUpp ) {
    const int index = _nCentralityBins*iEta + iCentrality;
    setY( (TGraphAsymmErrors*)_sigSyst6.At(index), iPt, value, errLow, errUpp );
  }
  void setSigSyst7( const int iPt, const int iEta, const int iCentrality, const double value, const double errLow, const double errUpp ) {
    const int index = _nCentralityBins*iEta + iCentrality;
    setY( (TGraphAsymmErrors*)_sigSyst7.At(index), iPt, value, errLow, errUpp );
  }
  void setSigSyst8( const int iPt, const int iEta, const int iCentrality, const double value, const double errLow, const double errUpp ) {
    const int index = _nCentralityBins*iEta + iCentrality;
    setY( (TGraphAsymmErrors*)_sigSyst8.At(index), iPt, value, errLow, errUpp );
  }
  void setSigSyst9( const int iPt, const int iEta, const int iCentrality, const double value, const double errLow, const double errUpp ) {
    const int index = _nCentralityBins*iEta + iCentrality;
    setY( (TGraphAsymmErrors*)_sigSyst9.At(index), iPt, value, errLow, errUpp );
  }
  void setSigSyst10( const int iPt, const int iEta, const int iCentrality, const double value, const double errLow, const double errUpp ) {
    const int index = _nCentralityBins*iEta + iCentrality;
    setY( (TGraphAsymmErrors*)_sigSyst10.At(index), iPt, value, errLow, errUpp );
  }
  ...
  void write( TFile& f ) {
    ...
    for ( int i = 0; i < _nEtaBins; i++ ) {
      for ( int j = 0; j < _nCentralityBins; j++ ) {
        ...
        ((TGraphAsymmErrors*)_sigSyst.At(index))->Write( (TString(_name)+="_sigSyst")+=s );
        ((TGraphAsymmErrors*)_sigSyst1.At(index))->Write( (TString(_name)+="_sigSyst1")+=s );
        ((TGraphAsymmErrors*)_sigSyst2.At(index))->Write( (TString(_name)+="_sigSyst2")+=s );
        ((TGraphAsymmErrors*)_sigSyst3.At(index))->Write( (TString(_name)+="_sigSyst3")+=s );
        ((TGraphAsymmErrors*)_sigSyst4.At(index))->Write( (TString(_name)+="_sigSyst4")+=s );
        ((TGraphAsymmErrors*)_sigSyst5.At(index))->Write( (TString(_name)+="_sigSyst5")+=s );
        ((TGraphAsymmErrors*)_sigSyst6.At(index))->Write( (TString(_name)+="_sigSyst6")+=s );
        ((TGraphAsymmErrors*)_sigSyst7.At(index))->Write( (TString(_name)+="_sigSyst7")+=s );
        ((TGraphAsymmErrors*)_sigSyst8.At(index))->Write( (TString(_name)+="_sigSyst8")+=s );
        ((TGraphAsymmErrors*)_sigSyst9.At(index))->Write( (TString(_name)+="_sigSyst9")+=s );
        ((TGraphAsymmErrors*)_sigSyst10.At(index))->Write( (TString(_name)+="_sigSyst10")+=s );
        ((TGraphAsymmErrors*)_nobs.At(index))->Write( (TString(_name)+="_nobs")+=s );
...
*/    
  ...
  void setY( TGraphAsymmErrors* graph, const int iPt, const double value, const double errLow, const double errUpp ) {
    double x, y;
    graph->GetPoint(iPt,x,y);
    //std::cout <<"Getting x: " << x << " y: " << y << std::endl;
    x = value; //temp hack
    graph->SetPoint(iPt,x,value);
    graph->SetPointEYlow(iPt,errLow);
    graph->SetPointEYhigh(iPt,errUpp);
    std::cout <<"Setting TGraph point " << iPt << " x: " << x << " y: " << value << " +/-" << errLow << std::endl;
//    exit(0); //temp hack
  }
...
//  w.setVal(wt);
//  std::cout << "Weighting data with weighting factor " << w.getVal() << std::endl;
...
// fillHIMuonDataSet
///////////////////////////////////////////////////////////////////////////////
RooDataSet* fillHIMuonDataSet(const TString& pathName, const TString& fileName, RooArgSet& muonArgSet, int cutValue=9,bool isMc = false,double wt=1.0 
     /*bool doPeriodA = false, bool doPeriodB = false, bool doPeriodC = false*/ )
{
  RooDataSet* set = new RooDataSet(fileName,fileName,muonArgSet);
  ...
  ///Nominal isolation definition
  tree->SetBranchAddress("ptcone20ID3", &ptconeNt);
  //systematic for ptcone
//  std::cout << "WARNING: Using increased cone size." << std::endl;
//  tree->SetBranchAddress("ptcone30ID3", &ptconeNt);
  ...
  //if(isMc) tree->SetBranchAddress("mTCB_smeared", &mtSmNt);
  ...
  tree->SetBranchAddress("centrality", &centralityNt);
  ///Uncomment for mpt resolution systematic study
//  std::cout << "WARNING: Using smeared mt for systematic study." << std::endl;
//  tree->SetBranchAddress("mtMod", &mtNt);
  //Nominal variable
...
//  tree->SetBranchAddress("nu_ptMod", &nu_ptNt);
/*  std::cout << "WARNING!WARNING!WARNING!: Using modified 2GeV track pT threshold for mpT sytematic study. Comment out if doing nominal analysis." << std::endl;
  tree->SetBranchAddress("nu_pt2000Nominal", &nu_ptNt);
  tree->SetBranchAddress("nu_phi2000Nominal", &nu_phiNt);
  */
/*  std::cout << "WARNING!WARNING!WARNING!: Using modified 4GeV track pT threshold for mpT sytematic study. Comment out if doing nominal analysis." << std::endl;
...
/*    
    if(doPeriodA&&!isMc){
        if(i==0) std::cout << "Running on runs < 193655." << std::endl;
        if(!(runNt<193655)) continue;
    } 
    else if(doPeriodB&&!isMc){
        if(i==0) std::cout << "Running on runs 193655 - 194017." << std::endl;
        if(!(runNt>=193655&&runNt<194017)) continue;
    } 
    else if(doPeriodC&&!isMc){
        if(i==0) std::cout << "Running on runs > 194017" << std::endl;
        if(!(runNt>=194017)) continue;
    }
*/
    if( (trig1)||(trig2)||(trig3)||isMc){
    for (int imu = 0; imu<nmu;imu++){
    if( ( (trig1&&matched1[imu]) || (trig2&&matched2[imu]) || (trig3&&matched3[imu]) ) ||isMc){
      if (isMc) {
      ...
      muonArgSet.setRealValue("run",runNt);
      muonArgSet.setRealValue("event",eventNt);
      muonArgSet.setRealValue("w",wt);
      ...
      double isolationMuTemp = -9999.0;
      //if(isMc)	isolationMuTemp = ptconeNt[imu]/ptSmNt[imu]; 
      isolationMuTemp = ptconeNt[imu]/ptNt[imu];
      muonArgSet.setRealValue("isolationMu",isolationMuTemp);
      //if(isMc) muonArgSet.setRealValue("muonPt",ptSmNt[imu]);
      muonArgSet.setRealValue("muonPt",ptNt[imu]);
      //if(isMc) muonArgSet.setRealValue("muonMt",mtSmNt[imu]);

      ///Nominal mt
      muonArgSet.setRealValue("muonMt",mtNt[imu]);
      ///hack for systematics 
      /*if(i==0&&imu==0)std::cout << "WARNING!WARNING!WARNING!: Using modified mt for sytematic study. Comment out if doing nominal analysis." << std::endl;
      float dPhi = phiNt[imu]-nu_phiNt;
      if(dPhi<-1*TMath::Pi()) dPhi += TMath::TwoPi(); if(dPhi>TMath::Pi()) dPhi -= TMath::TwoPi(); //fold btwn [-pi,pi]
      double mtTemp = fabs(nu_ptNt)<9000. ? TMath::Sqrt(2.0*nu_ptNt*ptNt[imu]*(1.0-TMath::Cos(dPhi))):-9999.;
      muonArgSet.setRealValue("muonMt",mtTemp);
     */ 
      double etaAbs = -9999.0;
      etaAbs = fabs(etaNt[imu]);
      // use etaAbs when plotting absolute eta for signal candidates 
      //muonArgSet.setRealValue("muonEta",etaAbs);
...
// selectPtEtaMpt
///////////////////////////////////////////////////////////////////////////////
RooDataSet* selectPtEtaMpt( RooDataSet* dataSet,double mptLow, double ptLow, double ptUpp, double etaLow, double etaUpp, bool doMirrorEta = false )
{
  TString cut = "muonPt>";
  cut += ptLow;
  cut += "&&muonPt<";
...
///////////////////////////////////////////////////////////////////////////////
// selectGenPtEtaCentrality
///////////////////////////////////////////////////////////////////////////////
RooDataSet* selectGenPtEtaCentrality( RooDataSet* dataSet, double ptLow, double ptUpp, double etaLow, double etaUpp, double centralityLow, double centralityHigh, bool doMirrorEta = false )
{
  dataSet = selectGenPtEta( dataSet, ptLow, ptUpp, etaLow, etaUpp, doMirrorEta);
...
float getMedian(float arr[],int size)
{
  ...
  std::cout << "Median of array is: " << average << std::endl;
  return average;
}
          ...
          TFile* fPdf, double pionWeight = 1.0, bool doParametricModels = false, bool onlyDistortBkg = false )
{
  if (dataSubSet->numEntries()==0) { /// early abort for no data scenario
    ...
    fitResult.setNobs(iPt,iEta, iCentrality,0,0.0,0.0);
...
//   }
  ...
  fitResult.setNobs(iPt,iEta, iCentrality,dataSubSet->numEntries(),0.0,0.0);
