...
TH1F* scaleByRaa(TH1F* hQCD, TH1F* hRAA, double ptSigLo, int icent){
    ...
    std::cout << "Scaling preselected histogram by the R_AA of centrality bin " << icent << std::endl;
        ...
        for(int iRAA = binRAALo; iRAA<=binRAAUpp; ++iRAA){
            ...
            ///Loop over each bin in the preselected and weighted QCD pt spectrum
...
void plot(TFile* outFile, RooRealVar& muonPt,RooDataSet* dataSet, RooDataSet* mcJ1Set, RooDataSet* mcJ2Set, RooDataSet* mcJ3Set, 
        ...
        double preSelectedQCD,preSelectedData,bkgQCDPreSel;
        if(doScaleByRaa){
        ...
        if(doScaleByRaa) hdataSetPSc->GetXaxis()->SetRangeUser(20.0,100.0);
...
void bkgQCDPlotter()
{
    ...
    if(doIsolationOptimization) std::cout << "Calculating QCD background with isolation optimization cuts." <<
        std::endl;
...
//	TString fileNameDataIn = baseString+"HardProbesFiles/HISingleMuonHP.12.19.2012";
	TString fileNameDataIn = "HardProbesFiles/HISingleMuonHardProbesData.04.17.2013";
...
//    fileNameDataGrIn = "background/fractionQCDEta_NoIsoCut_9Bins.06.24.2013";
    ...
    fileNameDataGrIn = "systematics/fractionQCDEta_MPTup_systematics.06.24.2013";
    ///mpt systematics -1sigma
//    fileNameDataGrIn = "systematics/fractionQCDEta_MPTdown_systematics.06.24.2013";
    //use for ptcone30ID3<0.1 systematics
//	fileNameDataGrIn = "systematics/fractionQCDEta_ptcone30ID3_systematics.04.21.2013";
    //use for ptcone20ID3<0.2 systematics
	//fileNameDataGrIn = "systematics/fractionQCDEta_ptcone20ID3_systematics.04.21.2013";
...
//	fileNameDataGrIn = "fractionQCDEta_1sigmaUpMPt.02.08.2013";
    ...
    cout <<  fileNameDataGrIn+".root" << " opened." << endl;
...
*/
      ...
      if((doMptSigmaDown||doMptSigmaUp))
          sCutsSig = "muonPt>4.0&&missPt<9000.0&&isolationMu<0.1&&muonMt>40.0&&muonMt<400.0&&ZDY==0&&muonQuality>11&&abs(muonELoss)<0.5&&abs(muonScat)<4.0";
      ///PS + Z veto + isolation
      else if(doIsolationOptimization) 
          sCutsSig =
          "muonPt>4.0&&missPt>25.0&&missPt<9000.0&&muonMt>40.0&&muonMt<400.0&&ZDY==0&&muonQuality>11&&abs(muonELoss)<0.5&&abs(muonScat)<4.0";
      else 
          sCutsSig = "muonPt>4.0&&missPt>25.0&&missPt<9000.0&&isolationMu<0.1&&muonMt>40.0&&muonMt<400.0&&ZDY==0&&muonQuality>11&&abs(muonELoss)<0.5&&abs(muonScat)<4.0";

      ///systematic for ptcone20ID3
//          "muonPt>4.0&&missPt>25.0&&missPt<9000.0&&isolationMu<0.2&&muonMt>40.0&&muonMt<400.0&&ZDY==0&&muonQuality>11&&abs(muonELoss)<0.5&&abs(muonScat)<4.0";

//	  TString sCutsSig = "muonPt>4.0&&missPt>25.0&&missPt<9000.0&&isolationMu<0.2&&muonMt>40.0&&muonMt<400.0&&ZDY==0";
//	  TString sCutsSig = "muonPt>4.0&&missPt>25.0&&missPt<9000.0&&muonMt>40.0&&muonMt<400.0&&ZDY==0";
	  //systematics
...
//	ncoll.push_back(1500.6); //0-10
    ...
    if(doMptSigmaDown&&doCentrality){
        ///-1sigma
        mptSyst.push_back(mptLowCut-14.1); //0-5%
        mptSyst.push_back(mptLowCut-13.5); //5-10%
        mptSyst.push_back(mptLowCut-13.2); //10-15%
        mptSyst.push_back(mptLowCut-12.4); //15-20%
        mptSyst.push_back(mptLowCut-11.2); //20-40%
        mptSyst.push_back(mptLowCut-9.2); //40-80%
        //for(int impt=0; impt<mptLow.size(); ++impt) std::cout << mptLow[impt] << std::endl;
    }
    else if(doMptSigmaDown) mptSyst.push_back(mptLowCut-12.6); //0-80%
    ...
    else if (doMptSigmaUp) mptSyst.push_back(mptLowCut+12.6); //0-80%
	...
	for ( int i = 0; i < nPtBins; i++ ) {
	  for ( int j = 0; j < nEtaBins; j++ ) {
	    for ( int k = 0; k < nCentralityBins; k++ ){
            ...
            if((doMptSigmaDown||doMptSigmaUp))  mcJ1SubSet[i][j][k] = selectMpt(mcJ1SubSet[i][j][k],mptSyst[k]); 
            ...
            if((doMptSigmaDown||doMptSigmaUp))  mcJ2SubSet[i][j][k] = selectMpt(mcJ2SubSet[i][j][k],mptSyst[k]); 
            ...
            if((doMptSigmaDown||doMptSigmaUp))  mcJ3SubSet[i][j][k] = selectMpt(mcJ3SubSet[i][j][k],mptSyst[k]); 
    ...
    //ptcone30ID3<0.1
/*	double bkgFracInt = 0.0296564;
	double bkgFracIntPlus = 0.0284271;
	double bkgFracIntMinus = 0.0309052;
*/
    //ptcone20ID3<0.2
...
*/
    ...
    if(doMptSigmaUp){
	 bkgFracInt = 0.00552;
	 bkgFracIntPlus = 0.00487;
	 bkgFracIntMinus = 0.00619;
    }
    ///mpt -1 sigma
    else if(doMptSigmaDown){
	 bkgFracInt = 0.0472;
	 bkgFracIntPlus = 0.0465;
	 bkgFracIntMinus = 0.0480;
    }
    else if(doIsolationOptimization){
	 bkgFracInt = 0.068708;
